...
import com.qa.todo.business.TodoService;
...
public class TodoController {
	...
	@GetMapping(path = Constants.TODO) //  @GetMapping(path = "/recipe")
	public ResponseEntity<List<Recipe>> getTodo() {
		return ResponseEntity.ok(todoService.getTodo());
	}
	
	@RequestMapping(value = Constants.TODO, method = RequestMethod.POST)
	public ResponseEntity<Recipe> postTodo(@RequestBody Recipe todo) {
		return ResponseEntity.ok(todoService.addTodo(todo));
	}

	@RequestMapping(value = Constants.TODO_WITH_PARAM, method = RequestMethod.DELETE)
	public ResponseEntity<List<Recipe>> deleteTodo(@PathVariable("id") long id) {
